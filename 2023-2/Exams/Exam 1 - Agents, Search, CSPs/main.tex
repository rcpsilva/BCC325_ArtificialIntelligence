...
\begin{enumerate}
    \item É obrigatória a entrega do código fonte dos algoritmos de busca em espaços de estados da última lista de exercícios.
    \item As linhas de código devem estar numeradas.
    \item A falha em cumprir as exigências acima acarretará em nota 0.
\end{enumerate}
...
\item Dado um problema de busca em espaços de estados $P$, responda:
\begin{enumerate}
    \item Quais características $P$ deve ter para que seja recomendado usar um algoritmo de busca em profundidade?
    \item Quais características $P$ deve ter para que seja recomendado usar um algoritmo de busca em largura?
    \item Quais características $P$ deve ter para que seja recomendado usar o algoritmo de menor custo primeiro?
    \item Quais características $P$ deve ter para que seja recomendado usar o algoritmo A$^*$?
\end{enumerate}

\item Considere a sua implementação dos algoritmos de busca em largura e busca em profundidade:
\begin{enumerate}
    \item Em relação à implementação, qual a diferença entre os dois? Indique em quais linhas de código está esta diferença.
    \item Como funciona o algoritmo de poda de ciclos? Caso você tenha implementado, indique em quais linhas de código a poda está implementada. Caso contrário, indique como o seu código deve ser alterado para a inclusão da poda. 
\end{enumerate}

\item Considere a sua implementação do algoritmo A$^*$.
\begin{enumerate}
    \item Indique a linhas de código que implementam a função heurística.
    \item A função heurística que você implementou é admissível? Explique.
    \item Discuta vantagens e desvantagens do A$^*$ em relação ao Branch and Bound.
\end{enumerate}
